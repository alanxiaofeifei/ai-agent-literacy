...
- **零 IT 基础学习者**：从白话解释、生活类比和安全边界开始；优先学习模型、幻觉、Agent Loop、工具、记忆与提示注入。
- **有通用 IT 技术基础、但不熟悉 AI 的学习者**：以软件、系统、网络、云、数据、安全、自动化和运维等通用 IT 场景理解 AI Agent；优先学习 API、Tool Calling、Workflow/Agent、Harness、MCP、上下文工程、Evals 与可观测性。

两条路线不是只换标题：切换后会显示不同的四步学习导览、默认展开层级、概念推荐顺序和 Hermes 模块顺序，并在时间轴中使用对应视角。两条路线仍保留全部64个概念和25个 Hermes 模块，方便随时交叉学习。
