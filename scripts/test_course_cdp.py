#!/usr/bin/env python3
"""Real Chromium interaction test over Chrome DevTools Protocol."""
...
async def main_async(args):
    ...
    async with CDP(target["webSocketDebuggerUrl"]) as cdp:
        ...
        assert "非官方转载" in resources["repostText"] and resources["safeLinks"]
        ...
        assert "重试 2 / 2" in loop["log"] and "安全停止" in loop["log"]
        ...
        assert "30" in quota and "教学模拟" in quota and "官方控制台" in quota
        ...
        assert "判断正确" in memory and "USER" in memory
        ...
        assert "不保存" in secret_memory and "撤销或轮换" in secret_memory
        ...
        assert "不保存" in redacted_memory and "撤销或轮换" in redacted_memory

        decision = await cdp.evaluate("const t=document.querySelector('input[name=decision-task]'); t.value='读取网页并整理公开资料'; document.querySelector('[data-testid=decision-start]').click(); ({text:document.querySelector('#decision-result').innerText,steps:document.querySelectorAll('#decision-result details').length})")
        checks["control_decision"] = decision
        assert decision["steps"] == counts["controlQuestions"] and "最小" in decision["text"]
        ...
        checks["quiz_feedback"] = {"id": quiz["id"], "has_explanation": "解释" in quiz["feedback"] or "正确" in quiz["feedback"] or "答案" in quiz["feedback"]}
        ...
        certificate_complete = await cdp.evaluate("""(()=>{
          ...
          for (const item of window.COURSE_DATA.quizzes.filter(x=>x.category==='安全').slice(0,5)) {
